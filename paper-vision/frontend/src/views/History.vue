  ...
  <div>
    ...
    <el-table :data="history" stripe>
      ...
      <el-table-column label="预览" width="120">
        <template #default="s"><img v-if="images[s.row.taskId]" :src="images[s.row.taskId]" style="width:60px;height:40px;cursor:pointer" @click="preview(s.row)" /></template>
      </el-table-column>
...
export default {
  ...
  async mounted() {
    ...
    for (const row of this.history) {
      if (row.taskId) {
        try { this.images[row.taskId] = URL.createObjectURL(await taskApi.image(row.taskId)) } catch (e) {}
      }
    }
  },
  methods: {
    preview(row) { this.previewImage = this.images[row.taskId]; this.previewVisible = true },
    async download(row) { const a = document.createElement('a'); a.href = URL.createObjectURL(await taskApi.image(row.taskId)); a.download = `${row.chartName || row.formulaName}.png`; a.click() },
    async del(row) { await taskApi.delHistory(row.id); this.$message.success('已删除'); this.history = await taskApi.history() }
  }
}
</script>
