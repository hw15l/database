<template>
  <div>
    ...
    <el-row :gutter="20">
      <el-col :span="6">
        <el-card>
          <template #header>选择图表类型（可多选，至少一个）</template>
          <el-checkbox-group v-model="selectedCharts" style="display:flex;flex-direction:column">
            <el-checkbox v-for="c in charts" :key="c.id" :label="c.id" style="margin:4px 0">
              {{ c.chartName }}
            </el-checkbox>
          </el-checkbox-group>
        </el-card>
      ...
      <el-col :span="18">
        <el-card>
          <template #header>渲染结果 <span style="color:#999;font-size:12px">(点击图片放大/下载)</span></template>
          <el-empty v-if="results.length === 0 && !generating" description="请勾选至少一种图表和数据源后点击生成" />
          <div v-loading="generating" style="display:flex;flex-wrap:wrap;gap:16px">
            <el-card v-for="r in results" :key="r.taskId" shadow="hover" style="width:calc(50% - 8px)">
              ...
              <div v-if="r.isHtml" style="text-align:center;padding:20px">
                ...
                <p style="font-size:12px;color:#909399">交互式图表 (HTML)</p>
...
export default {
  ...
  async mounted() {
    const [c, f] = await Promise.all([chartApi.list(), dataApi.files()])
    this.charts = c.data; this.files = f.data
  },
  methods: {
    async generateBatch() {
      ...
      this.results = []
      try {
        const params = { colorScheme: this.colorScheme, dpi: this.dpi }
        const r = await chartApi.generateBatch({ chartIds: this.selectedCharts, fileId: this.selectedFile, params })
        for (const task of r.data) {
          ...
          if (task.status === 'SUCCESS' && task.resultPath) {
            if (task.resultPath.endsWith('.html')) {
              item.isHtml = true; item.htmlTaskId = task.id
            } else {
              try { const img = await taskApi.image(task.id); item.image = URL.createObjectURL(img.data) } catch (e) {}
            }
          }
